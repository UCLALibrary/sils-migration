...
# Extracts "reported for deletion" bibs with OCLC numbers from all 3 Voyager databases,
...
for DB in ethnodb filmntvdb ucladb; do
  ...
  # Query to get db-specific bib record ids: records reported to OCLC for deletion
  SQL_FILE=${OUT_DIR}/reported_${DB}_${TYPE}.sql
  (
    echo "set linesize 10;"
	echo "select record_id"
	echo "from vger_subfields.${DB}_bib_subfield"
    echo "where tag = '936a'"
    echo "and upper(subfield) like 'RECORD REPORTED FOR DELETION%'"
	echo "order by record_id;"
  ) > ${SQL_FILE}
  ...
  EXTRACT_FILE=${OUT_DIR}/reported_${DB}_${TYPE}.mrc
  ...
  python3 ${DIR}/process_vanguard_reported_bibs.py ${EXTRACT_FILE} ${UPDATE_FILE}
